...
define("guiprocessmanager", ["require", "exports", "colorreductionmanagement", "common", "facetBorderSegmenter", "facetBorderTracer", "facetCreator", "facetLabelPlacer", "facetmanagement", "facetReducer", "gui", "structs/point"], function (require, exports, colorreductionmanagement_2, common_7, facetBorderSegmenter_1, facetBorderTracer_1, facetCreator_3, facetLabelPlacer_1, facetmanagement_4, facetReducer_1, gui_1, point_5) {
    ...
    class GUIProcessManager {
                ...
                for (const f of facetResult.facets) {
                    if (f != null && f.borderSegments.length > 0) {
                        ...
                        if (addColorLabels) {
                            ...
                            const labelText = GUIProcessManager.convertNumberToLabel(f.color);
        ...
        static convertNumberToLabel(colorIndex) {
            // Lấy chữ cái được chọn cho màu này từ colorLabels (nếu có)
            const letterSuffix = (GUIProcessManager.colorLabels && GUIProcessManager.colorLabels[colorIndex]) 
                ? GUIProcessManager.colorLabels[colorIndex] 
                : "";
                
            // Hiển thị màu từ 1 thay vì 0 (colorIndex + 1)
            return (colorIndex + 1) + letterSuffix;
        }
    }
    
    // Khởi tạo biến colorLabels để lưu các tùy chọn chữ cái cho từng màu
    GUIProcessManager.colorLabels = {};
...
define("gui", ["require", "exports", "common", "guiprocessmanager", "settings"], function (require, exports, common_8, guiprocessmanager_1, settings_2) {
    ...
    function parseSettings() {
        ...
        // Xóa tất cả các nhãn hiện tại
        guiprocessmanager_1.GUIProcessManager.colorLabels = {};
        
        // Số lượng màu
        const numColors = settings.kMeansNrOfClusters;
        
        // Đọc các tùy chọn chữ cái cho từng màu
        for (let i = 0; i < 16; i++) {
            // Chỉ áp dụng cho các màu trong phạm vi số lượng màu đã chọn
            if (i < numColors) {
                const letterValue = $(`#color${i}Letter`).val();
                if (letterValue && letterValue !== "") {
                    guiprocessmanager_1.GUIProcessManager.colorLabels[i] = letterValue;
                }
            }
        }
        
        console.log("Color labels applied:", guiprocessmanager_1.GUIProcessManager.colorLabels);
    ...
    function updateOutput() {
        return __awaiter(this, void 0, void 0, function* () {
            if (processResult != null) {
                ...
                // In ra log để debug
                console.log("Current color labels:", guiprocessmanager_1.GUIProcessManager.colorLabels);
            }
        });
    }
    ...
    function createPaletteHtml(colorsByIndex) {
        ...
        for (let c = 0; c < colorsByIndex.length; c++) {
            ...
            const labelText = guiprocessmanager_1.GUIProcessManager.convertNumberToLabel(c);
            html += `<div class="color" class="tooltipped" style="${style}" data-tooltip="${colorsByIndex[c][0]},${colorsByIndex[c][1]},${colorsByIndex[c][2]}" data-color-index="${c}">${labelText}</div>`;
        }
        return $(html);
    }

    // Hàm tạo giao diện cấu hình chữ cái cho từng màu
    function createColorLetterConfig(colorsByIndex) {
        let html = "";
        // Lấy các chữ cái đã chọn từ dropdown hoặc mặc định
        const letterOptions = $("#letterOptions").val() || ["A", "B", "C", "D"];
        
        // Style cho container
        html += `<div style="max-height: 400px; overflow-y: auto; margin-bottom: 20px;">`;
        
        for (let c = 0; c < colorsByIndex.length; c++) {
            const rgb = colorsByIndex[c];
            const currentLetter = guiprocessmanager_1.GUIProcessManager.colorLabels[c] || "";
            
            html += `<div class="color-letter-config" style="margin: 5px 0; padding: 10px; border-bottom: 1px solid #eee; display: flex; align-items: center;">`;
            // Ô màu
            html += `<div style="display: inline-block; width: 30px; height: 30px; background-color: rgb(${rgb[0]},${rgb[1]},${rgb[2]}); margin-right: 10px; border: 1px solid #ddd;"></div>`;
            // Số màu
            html += `<span style="min-width: 80px;">Màu ${c}: </span>`;
            // Dropdown chọn chữ cái
            html += `<div class="input-field" style="margin: 0; flex-grow: 1;">`;
            html += `<select class="letter-select browser-default" data-color-index="${c}">`;
            html += `<option value="" ${currentLetter === "" ? "selected" : ""}>Không có chữ cái</option>`;
            
            for (const letter of letterOptions) {
                html += `<option value="${letter}" ${currentLetter === letter ? "selected" : ""}>${letter}</option>`;
            }
    ...
    // Hàm áp dụng các chữ cái đã chọn
    function applyLetterSelections() {
        // Xóa tất cả các chữ cái hiện tại
        guiprocessmanager_1.GUIProcessManager.colorLabels = {};
        
        // Thu thập các chữ cái đã chọn
        $(".letter-select").each(function() {
            const colorIndex = parseInt($(this).data("color-index"));
            const letterValue = $(this).val();
            
            if (letterValue && letterValue !== "") {
                guiprocessmanager_1.GUIProcessManager.colorLabels[colorIndex] = letterValue;
            }
        });
        
        console.log("Applying letters:", guiprocessmanager_1.GUIProcessManager.colorLabels);
    ...
    function downloadPalettePng() {
        ...
        for (let i = 0; i < colorsByIndex.length; i++) {
            ...
            // Sử dụng hàm convertNumberToLabel để hiển thị nhãn
            const labelText = guiprocessmanager_1.GUIProcessManager.convertNumberToLabel(i);
            ctx.fillStyle = "black";
            ctx.strokeStyle = "#CCC";
            ctx.font = "20px Tahoma";
            const nrTextSize = ctx.measureText(labelText);
            ctx.lineWidth = 2;
            ctx.strokeText(labelText, x + cellWidth / 2 - nrTextSize.width / 2, y + cellHeight / 2 - 5);
            ctx.fillText(labelText, x + cellWidth / 2 - nrTextSize.width / 2, y + cellHeight / 2 - 5);
            ctx.lineWidth = 1;
...
define("main", ["require", "exports", "gui", "lib/clipboard"], function (require, exports, gui_2, clipboard_1) {
    ...
    $(document).ready(function () {
        $(".tabs").tabs();
        $(".tooltipped").tooltip();
        // Khởi tạo tất cả các select box
        $('select').formSelect();
        
        const clip = new clipboard_1.Clipboard("canvas", true);
        
        // Khi số lượng màu thay đổi, cập nhật giao diện tùy chọn nhãn màu
        $("#txtNrOfClusters").change(function() {
            const numColors = parseInt($(this).val());
            
            // Ẩn/hiện các select box tùy thuộc vào số lượng màu
            for (let i = 0; i < 16; i++) {
                const colorSelect = $(`#color${i}Letter`).closest('.col');
                if (i < numColors) {
                    colorSelect.show();
                } else {
                    colorSelect.hide();
        ...
        // Kích hoạt sự kiện change khi trang vừa tải để ẩn/hiện các select box phù hợp
        $("#txtNrOfClusters").trigger("change");
        ...
        // Đăng ký sự kiện cho nút áp dụng chữ cái
        $(document).on("click", "#btnApplyLetters", function() {
            console.log("Apply letters button clicked");
            applyLetterSelections();
        });
        
        // Sự kiện khi thay đổi các tùy chọn chữ cái
        $(document).on("change", "#letterOptions", function() {
            if (processResult != null) {
                $("#colorLetterConfig").empty().append(createColorLetterConfig(processResult.colorsByIndex));
            }
        });
