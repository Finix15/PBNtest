...
define("guiprocessmanager", ["require", "exports", "colorreductionmanagement", "common", "facetBorderSegmenter", "facetBorderTracer", "facetCreator", "facetLabelPlacer", "facetmanagement", "facetReducer", "gui", "structs/point"], function (require, exports, colorreductionmanagement_2, common_7, facetBorderSegmenter_1, facetBorderTracer_1, facetCreator_3, facetLabelPlacer_1, facetmanagement_4, facetReducer_1, gui_1, point_5) {
    ...
    class GUIProcessManager {
        ...
        static convertNumberToLabel(colorIndex) {
            // Lấy loại nhãn được chọn
            const labelType = document.getElementById("selectLabelType").value;
            
            // Xử lý theo loại nhãn
            if (labelType === "letters") {
                // Chuyển đổi thành chữ cái (A-Z, a-z)
                if (colorIndex < 26) {
                    return String.fromCharCode(65 + colorIndex); // A-Z
                } else if (colorIndex < 52) {
                    return String.fromCharCode(97 + (colorIndex - 26)); // a-z
                } else {
                    // Kết hợp các chữ cái cho các số lớn hơn (AA, AB,...)
                    const firstChar = String.fromCharCode(65 + Math.floor(colorIndex / 26) - 1);
                    const secondChar = String.fromCharCode(65 + (colorIndex % 26));
                    return firstChar + secondChar;
                }
            } else if (labelType === "numbers") {
                // Sử dụng số như ban đầu
                return (colorIndex + 1) + ""; // +1 để bắt đầu từ 1 thay vì 0
            } else if (labelType === "custom") {
                // Sử dụng nhãn tùy chỉnh từ textarea
                const customLabels = document.getElementById("txtCustomLabels").value.split('\n')
                    .map(label => label.trim())
                    .filter(label => label.length > 0);
                
                if (customLabels.length > 0) {
                    // Sử dụng nhãn tùy chỉnh nếu có, hoặc lặp lại từ đầu nếu không đủ nhãn
                    return customLabels[colorIndex % customLabels.length];
                } else {
                    // Nếu không có nhãn tùy chỉnh nào được cung cấp, sử dụng số
                    return (colorIndex + 1) + "";
                }
            }
            
            // Fallback mặc định: sử dụng số
            return (colorIndex + 1) + "";
        }
    ...
    define("gui", ["require", "exports", "common", "guiprocessmanager", "settings"], function (require, exports, common_8, guiprocessmanager_1, settings_2) {
        ...
        function createPaletteHtml(colorsByIndex) {
            ...
            for (let c = 0; c < colorsByIndex.length; c++) {
                ...
                html += `<div class="color" class="tooltipped" style="${style}" data-tooltip="${colorsByIndex[c][0]},${colorsByIndex[c][1]},${colorsByIndex[c][2]}">${labelText}</div>`;
            }
            return $(html);
        }
