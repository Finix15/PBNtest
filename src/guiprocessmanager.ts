...
export class GUIProcessManager {
        ...
        for (const f of facetResult.facets) {
            ...
            if (f != null && f.borderSegments.length > 0) {
                ...
                if (addColorLabels) {
                    ...
                    const labelText = this.convertNumberToLabel(f.color);
    ...
    /**
     * Chuyển đổi số thành ký tự nhãn
     * Có thể tùy chỉnh để sử dụng chữ cái, ký tự đặc biệt hoặc bất kỳ ký tự nào
     */
    private static convertNumberToLabel(colorIndex: number): string {
        // Phương pháp mặc định: trả về số như trước đây
        // return colorIndex + "";
        
        // Phương pháp 1: Chuyển thành chữ cái (A, B, C, ...)
        // Lưu ý: Chỉ hoạt động cho 26 màu đầu tiên
        if (colorIndex < 26) {
        ...
        // Phương pháp 2: Sử dụng ký tự đặc biệt
        // const specialChars = "!@#$%^&*()_+[]{}|;:,.<>?";
        // if (colorIndex < specialChars.length) {
        //     return specialChars[colorIndex];
        // }
        
        // Nếu vượt quá số lượng ký tự có sẵn, trở lại số
        return colorIndex + "";
    }
}
